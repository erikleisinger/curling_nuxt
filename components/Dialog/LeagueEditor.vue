<template>
    <q-dialog v-model="isOpen" @hide="close" persistent class="dialog">
        <q-card class="league-editor__wrap">
            ...
            <q-card-section>
                <h4>Draw times</h4>
                <h5 class="text-sm text-red" v-if="drawError">
                    {{ drawError }}
                </h5>
                <q-list separator>
                    <transition-group
                        ...
                        >
                            ...
                            <q-item-section avatar>
                                <q-btn
                                    ...
                                    dense
                                    icon="close"
                                    @click="drawtimes.splice(index, 1)"
                                />
                            </q-item-section>
                        </q-item>
                    </transition-group>
            ...
            <q-card-section>
                ...
                <div v-if="pools.length">
                    ...
                    >
                        <div
                            class="pool__container"
                            v-for="({ id: poolId, pool }, poolIndex) in pools"
                            :key="poolIndex"
                        >
                            ...
                            <q-list separator>
                                ...
                                <q-item
                                    ...
                                    v-ripple
                                    @click="
                                        toggleTeamSearch(pool?.id || poolIndex)
                                    "
                                >
                                    <q-item-section avatar>
                                        <q-icon name="add" flat round />
            ...
            <q-card-section>
                <h4>Admins</h4>
                <h5 class="text-sm text-red" v-if="adminError">
                    {{ adminError }}
                </h5>
                <q-list separator>
                    <transition-group
                    ...
                    >
                        ...
                        >
                            <q-item-section avatar>
                                <div style="width: 25px">
                                    <Avataaar
                                        v-bind="parseAvatar(admin.avatar)"
                                    />
                                </div>
                            </q-item-section>
                            <q-item-section>
                    ...
                    >
                        <q-item-section>
                            ...
                            <q-item-label caption
                                >Anyone can join this league.</q-item-label
                            >
                        </q-item-section>
                        <q-item-section avatar>
                            <q-toggle
...
const addDrawTime = (time: string, id: number) => {
    ...
    if (drawtimes.value.includes(utc)) {
        useNotificationStore().addNotification({
            state: "failed",
            text: `There's already a draw scheduled at ${toTimezone(
                utc,
                "h:mm a"
            )}!`,
            timeout: 3000,
...
const toggleProfileSearch = () => {
    toggleGlobalSearch({
        open: true,
        options: {
            inputLabel: `Add an admin for ${
                league.value?.name ?? "your league"
            }`,
    ...
    if (id) {
        ...
        if (index !== -1) {
            ...
            teams.value = [...teams.value].filter(
                ({ league_pool_id }) => league_pool_id !== id
            );
        }
    }
    pools.value.splice(index, 1);
    teams.value = [...teams.value].filter(
        ({ league_pool_id }) => league_pool_id !== index
    );
...
const poolsValid = computed(() => {
    return pools.value.every(({ id }, index) =>
        teams.value.some(
            ({ league_pool_id }) =>
                league_pool_id === id || league_pool_id === index
        )
    );
});
...
const createDrawTimes = async (league_id: number) => {
    await client
        .from("league_drawtimes")
        .delete()
        .eq("league_id", league_id)
        .not("id", "in", `(${drawtimes.value.map(({ id }) => id).join(",")})`);
    const { error } = await client.from("league_drawtimes").upsert(
        drawtimes.value.map(({ id, time }) => ({
            ...(id ? { id } : {}),
            league_id,
            time,
        }))
...
const createLeagueAdmins = async (league_id: number) => {
    await client
        ...
        .delete()
        .eq("league_id", league_id)
        .not("id", "in", `(${admins.value.map(({ id }) => id).join(",")})`);
    const { error } = await client.from("league_admins").upsert(
        admins.value.map(({ id, profile }) => ({
            ...(id ? { id } : {}),
            league_id,
...
const createLeaguePools = async (league_id: number) => {
    ...
    const { error, data: fetchedPools } = await client
        .from("league_pools")
        .upsert(
            pools.value.map(({ id, pool }) => ({
                ...(id ? { id } : {}),
                league_id,
                name: pool.name,
                format: pool.format,
...
const save = async () => {
    ...
    const notId = useNotificationStore().addNotification({
        state: "pending",
        text: `${isEdited.value ? "Updating" : "Creating"} ${
            league.value.name
        }...`,
    });
    try {
        ...
        await Promise.all([
            createDrawTimes(league_id),
            createLeagueTeams(league_id, poolIds),
        ]);
        ...
        useNotificationStore().updateNotification(notId, {
            state: "completed",
            text: `${league.value.name} was ${
                isEdited.value ? "updated" : "created"
            }!`,
            timeout: 2000,
        });
        ...
        useNotificationStore().updateNotification(notId, {
            state: "failed",
            text: `Error ${isEdited.value ? "updating" : "creating"} ${
                league.value.name
            }: ${error?.message}`,
            timeout: 5000,
        });
...
const initEdit = () => {
    ...
    teams.value = editedItem.value.teams.map(
        ({ team, id, league_pool_id }) => ({
            id,
            league_pool_id,
            team,
        })
