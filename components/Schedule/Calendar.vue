<template>
  <div class="calendar-container">
    <header class="row justify-between calendar-header--outer">
      <div class="title">
        ...
        <h3 class="text-sm">{{ currentRange }}</h3>
      </div>
      <nav class="row justify-between">
        <q-btn dense flat round icon="navigate_before" @click="api.prev()" />
        <q-btn dense flat round icon="navigate_next" @click="api.next()" />
      </nav>
    </header>
    <FullCalendar :options="calendarOptions" ref="calendar" id="#schedule-calendar">
      <template v-slot:dayHeaderContent="d">
        <div class="calendar-block" v-if="currentView === 'dayGridDay'"/>
        <h2 class="calendar-header--inner" :class="{today: d.isToday,  stretch: currentView === 'dayGridDay' }" @click="onHeaderClick(d)">
            <div class="today--indicator" v-if="d.isToday"/>
            <div class="header--inner__content row justify-center items-center">
          {{ getHeaderContent(d) }}
            </div>
        </h2>
      </template>
  ...
  <q-menu :target="eventTarget" :modelValue="!!eventTarget">
    <q-card class="event-viewmore__container">
        <q-card-section :style="{backgroundColor: eventViewing?.backgroundColor}">
        <h3 class="text-bold text-md text-white" >{{eventViewing?.title}}</h3>
        </q-card-section>
        <q-card-section>
            <div>
                <h4 class="text-sm text-bold">Start time</h4>
            {{dayjs(eventViewing.start).format('MMM DD, YYYY [at] HH:mm a')}}
            </div>
             <div>
                <h4 class="text-sm text-bold">End time</h4>
            {{dayjs(eventViewing.end).format('MMM DD, YYYY [at] HH:mm a')}}
            </div>
            <div v-if="eventViewing?.extendedProps?.team_id">
                 <div style="width: 25px">
                <TeamAvatar :teamId="eventViewing?.extendedProps?.team_id"/>
                 </div>
            </div>
        </q-card-section>
    </q-card>
  </q-menu>
  <DialogScheduler v-model="schedulerOpen" :date="selectedDate"/>
</template>
...
const getEvents = async () => {
    ...
    const userTeams = useUserTeamStore().userTeams.map(({id}) => id).join(',')
    console.log('user teams: ', userTeams)
    const client = useSupabaseClient();
    const {data} = await client.from('schedule').select('*').or(`profile_id.eq.${userId.value},team_id.in.(${userTeams})`)
...
const {isLoading, data: events} = useQuery({
    ...
    select: (val) => {
        ...
        console.log('events: ', api.value.getEvents())
        }, 100)
