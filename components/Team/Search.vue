     ...
     <div class="row q-pa-md" v-if="!viewing">
                ...
                >
                    ...
                    <q-item-section side @click.stop.prevent >
                        <div class="text-grey-8 row no-wrap">
                            <q-btn text-color="deep-purple" flat @click="viewUser(item)">View</q-btn>
                        </div>
                    </q-item-section>
                </q-item>
            </div>
        ...
        <div v-else-if="teams?.length" >
            <header class="row no-wrap">
                <q-btn flat round icon="arrow_back" @click="unView"/> <h1 class="text-md text-bold q-pa-sm"> <ProfileChip v-bind="viewedUser"/>'s teams</h1>
            </header>
        <TeamList  :teams="teams" @update="onItemUpdate"/>
        </div>
</template>
<style lang="scss">
.search__container {
...
const search = useThrottleFn(async () => {
    ...
    results.value = data.filter(({id}) => id !== userId.value);
}, 5000);
...
const viewUser = useThrottleFn(async ({id, username}) => {
    viewedUser.value = {
             id,
        username,
