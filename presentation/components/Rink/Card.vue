...
const fetchEnabled = computed(() => !!props.rinkId)

const fetchFunc = computed(() => getRink(props.rinkId))

const {loading, result: rink} = fetch(fetchFunc, {
    enabled: fetchEnabled
})
const glob = import.meta.glob("~/presentation/assets/flags/*", {
    eager: true,
});

const backgroundUrl = computed(() => {
    try {
        return glob[`/presentation/assets/flags/${rink.value?.province}.svg`]["default"];
    } catch {
        return "";
    }
});
